...
class Metadata(BaseModel):
    ...
    def actual_instance_must_validate_oneof(cls, v):
        ...
        if match > 1:
            # more than 1 match
            raise ValueError(
                "Multiple matches found when setting `actual_instance` in Metadata with oneOf schemas: MetadataOneOf, MetadataOneOf1. Details: "
                + ", ".join(error_messages)
            )
        elif match == 0:
            # no match
            raise ValueError(
                "No match found when setting `actual_instance` in Metadata with oneOf schemas: MetadataOneOf, MetadataOneOf1. Details: "
                + ", ".join(error_messages)
            )
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        if match > 1:
            # more than 1 match
            raise ValueError(
                "Multiple matches found when deserializing the JSON string into Metadata with oneOf schemas: MetadataOneOf, MetadataOneOf1. Details: "
                + ", ".join(error_messages)
            )
        elif match == 0:
            # no match
            raise ValueError(
                "No match found when deserializing the JSON string into Metadata with oneOf schemas: MetadataOneOf, MetadataOneOf1. Details: "
                + ", ".join(error_messages)
            )
        else:
            return instance
