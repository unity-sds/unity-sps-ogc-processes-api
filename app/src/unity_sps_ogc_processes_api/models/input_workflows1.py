...
INPUTWORKFLOWS1_ONE_OF_SCHEMAS = [
...
class InputWorkflows1(BaseModel):
    ...
    def actual_instance_must_validate_oneof(cls, v):
        instance = InputWorkflows1.model_construct()
        error_messages = []
        match = 0
        ...
        if match > 1:
            # more than 1 match
            raise ValueError(
                "Multiple matches found when setting `actual_instance` in InputWorkflows1 with oneOf schemas: InlineOrRefDataWorkflows, List[InlineOrRefDataWorkflows]. Details: "
                + ", ".join(error_messages)
            )
        elif match == 0:
            # no match
            raise ValueError(
                "No match found when setting `actual_instance` in InputWorkflows1 with oneOf schemas: InlineOrRefDataWorkflows, List[InlineOrRefDataWorkflows]. Details: "
                + ", ".join(error_messages)
            )
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        if match > 1:
            # more than 1 match
            raise ValueError(
                "Multiple matches found when deserializing the JSON string into InputWorkflows1 with oneOf schemas: InlineOrRefDataWorkflows, List[InlineOrRefDataWorkflows]. Details: "
                + ", ".join(error_messages)
            )
        elif match == 0:
            # no match
            raise ValueError(
                "No match found when deserializing the JSON string into InputWorkflows1 with oneOf schemas: InlineOrRefDataWorkflows, List[InlineOrRefDataWorkflows]. Details: "
                + ", ".join(error_messages)
            )
        else:
            return instance
