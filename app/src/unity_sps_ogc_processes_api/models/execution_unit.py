...
class ExecutionUnit(BaseModel):
    ...
    __properties: ClassVar[List[str]] = ["type", "image", "deployment", "config"]

    @field_validator("type")
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in ("docker", "oci"):
            raise ValueError("must be one of enum values ('docker', 'oci')")
        return value

    @field_validator("deployment")
    def deployment_validate_enum(cls, value):
        """Validates the enum"""
        if value is None:
            return value
        ...
        _obj = cls.model_validate(
            {
                ...
                "config": (
                    ExecutionUnitConfig.from_dict(obj.get("config"))
